...
export const ModificarViaje = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [viajeResponse, vehResponse, condResponse] = await Promise.all([
        ...
        if (!viajeResponse.ok || !viajeData.success) {
        ...
        setValues({
          vehiculo_id: viaje.vehiculo_id.toString(),
          conductor_id: viaje.conductor_id.toString(),
          fecha_salida: fechaSalida,
          fecha_llegada: fechaLlegada,
          origen: viaje.origen,
          destino: viaje.destino,
          kilometros: viaje.kilometros.toString(),
  ...
  const handleSubmit = async (e) => {
    ...
    if (!response.ok || !data.success) {
      if (response.status === 400) {
        ...
        return window.alert(data.error || "Error al modificar viaje");
      }
